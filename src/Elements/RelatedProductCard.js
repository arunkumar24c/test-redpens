...
function ProductCard(props) {
  ...
  useEffect(() => {
    ...
    const {product_name,product_image ,product_price_inr , product_price_usd, product_short_description, product_url, key  } = props.related_product
   ...
   console.log(props, "card data")
    ...
    const bg_color =props.related_product && props.related_product.shop_categories.chip_background_colors.color_code
    const text_color =props.related_product && props.related_product.shop_categories.chip_text_colors.color_code
  ...
  return (
    <div onClick={()=>{navigate(`/shopdetail/${product_url}`,{state:props})}}  className='mb-10 cursor-pointer  border-brnd-border md:w-2/6 rounded-xl flex  flex-col shrink-0 justify-between shadow-md hover:shadow-xl'>
                    ...
                    {/* <svg xmlns="http://www.w3.org/2000/svg" width="80" height="71" viewBox="0 0 80 71" fill="none">
                     ...
                     <div className='p-5 flex  flex-col gap-y-5'>
                      <p style={{background: `${bg_color}`, color: `${text_color}`}} className={`py-2 px-3 w-fit rounded-md text-xs `}>{props.related_product.shop_categories.category_name}</p>
                    ...
                    <p className=' text-base'>{product_short_description}</p>
                    </div>
                <div className=' rounded-b-xl bg-brnd-gray p-5 flex justify-between shrink-0 items-center  '>

                    {/* { region && region === 'India'  ?  */}
                    <div>
                    <p className='font-regular'> {region && region === 'India'  ? `₹${product_price_inr.toLocaleString("en-US")}` : `$${product_price_usd}` }</p>
